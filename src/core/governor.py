...
Keep tasks focused and minimal. Prefer fewer, well-defined tasks over many small ones."""
    ...
    async def _handle_agent_assignment(self, context: FSMContext) -> ExecutionState:
        """Assign agents to tasks with intelligent selection and fallback."""
        goal = context.goal_context.goal
        ...
        for task in goal.tasks:
            if task.assigned_agent_type:
                ...
                # Parse agent type with fallback
                try:
                    agent_type = AgentType(task.assigned_agent_type.lower())
                except ValueError:
                    self.logger.warning(
                        "Unknown agent type, defaulting to TOOL",
                        requested_type=task.assigned_agent_type,
                    )
                    agent_type = AgentType.TOOL

                # Strategy 1: Find existing available agent using intelligent scoring
                available_agents = list(self.registry._agents.values())
                if available_agents:
                    agent, score, rationale = await self.registry.find_best_agent(
                        agent_type, task, available_agents
                    )

                # Strategy 2: Create new agent if none found or score too low
                if agent is None or score < 0.3:
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            agent = await self.registry.create_agent(agent_type)
                            score = 0.5  # Default score for new agents
                            rationale = f"New agent created (attempt {attempt + 1})"
                            break
                        except Exception as e:
                            self.logger.warning(
                                "Agent creation attempt failed",
                ...
                if agent:
                    ...
                    # Mark task as failed if no agent could be assigned
                    task.status = TaskStatus.FAILED
                    task.error = f"Failed to assign agent of type {agent_type.value}"
                    self.logger.error(
        ...
        assigned_count = sum(1 for t in goal.tasks if t.assigned_agent_id)
        failed_count = len(assignment_failures)
        self.logger.info(
        ...
        # If all assignments failed, go to FAILED state
        if assigned_count == 0:
            return ExecutionState.FAILED

        # Decide execution pattern based on task count and dependencies
        assigned_tasks = [t for t in goal.tasks if t.assigned_agent_id]
    ...
    async def _handle_parallel_execution(self, context: FSMContext) -> ExecutionState:
        """Execute multiple tasks in parallel with dependency tracking."""
        goal = context.goal_context.goal
        ...
        completed_task_ids = {
            t.id for t in goal.tasks if t.status == TaskStatus.COMPLETED
        }
        ...
        for task in pending_tasks:
            ...
            elif all(dep_id in completed_task_ids for dep_id in task.dependencies):
                ready_tasks.append(task)
        ...
        if not ready_tasks:
            ...
            if pending_tasks:
                ...
                # Force execute remaining tasks to break deadlock
                ready_tasks = pending_tasks[:3]  # Execute up to 3 at a time
            else:
                return ExecutionState.VERIFYING

        # Group tasks by parallel_group for wave execution
        task_groups: dict[int, list[Task]] = {}
        for task in ready_tasks:
            group = getattr(task, 'parallel_group', 0) or 0
        ...
        # Execute each group in order, tasks within group run in parallel
        for group_id in sorted(task_groups.keys()):
            group_tasks = task_groups[group_id]

            # Execute all tasks in this group in parallel
            coroutines = [
                self._execute_task(task, context.goal_context)
        ...
        if remaining_pending:
            # More tasks to execute - continue parallel execution
            return ExecutionState.PARALLEL_EXECUTING
        else:
            return ExecutionState.VERIFYING
