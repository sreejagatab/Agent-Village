...
class AgentManager:
    ...
    async def find_best_agent(
        ...
        agent_type: AgentType,
        task: Task,
        available_agents: list[BaseAgent] | None = None,
        ...
        Args:
            ...
            available_agents: Currently available agent instances (optional, uses internal list if not provided)

        Returns:
            Tuple of (best_agent, score, rationale)
        """
        # Use internal agents if not provided
        ...
        try:
            ...
            for decision in past_decisions:
                ...
                if isinstance(content, dict):
                    agent_id = content.get("selected_agent_id")
                    if agent_id:
                        if content.get("outcome") == "success":
                            preferred_agents.add(agent_id)
                        elif content.get("outcome") == "failure":
                            avoided_agents.add(agent_id)
        except Exception as e:
            self.logger.warning("Failed to query strategic memory", error=str(e))
        ...
        for agent in available_agents:
            # Skip agents of wrong type
            if agent.agent_type != agent_type:
                continue

            # Skip agents that failed on similar tasks (unless no other option)
            if agent.id in avoided_agents and len(available_agents) > len(avoided_agents):
                continue

            profile = self._profiles.get(agent.id)
            if profile:
                # Use profile-based scoring
                base_score = profile.get_score_for_task(task_description, task_type)

                # Penalty for low success rate
                if profile.performance.total_tasks >= 3:
                    if profile.performance.success_rate < 0.5:
                        base_score *= 0.7  # Reduce score for poor performers
            ...
            # Bonus for agents that are currently idle
        ...
        if not scored_agents:
            return None, 0.0, f"No agents of type {agent_type.value} available"
        ...
        rationale_parts = [f"Selected {best_agent.name} (score: {best_score:.2f})"]
        if best_agent.id in preferred_agents:
            rationale_parts.append("past success on similar tasks")
        if best_score >= 0.8:
            rationale_parts.append("high capability match")
        rationale_parts.append(f"{task_type} task")
        ...
        self.logger.info(
            ...
            preferred=best_agent.id in preferred_agents,
        )
