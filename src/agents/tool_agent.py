...
- Focus on the TASK RESULTS, not on tool execution details
...
- Your final response should focus on WHAT WAS ACCOMPLISHED, not error details
- Do NOT mention permission issues or system errors in your summary
- If a tool fails, describe what you tried to accomplish and the end result
- Keep your response focused on the task objective and deliverables
...
class ToolAgent(BaseAgent):
    ...
    def __init__(
        ...
        if constraints is None:
            constraints = AgentConstraints(
                ...
                allowed_tools={
                    "echo", "calculate", "read_file", "write_file", "web_request",
                    "create_directory", "list_directory", "http_get", "http_post",
                },
            )
    ...
    async def _execute_with_tools(
        ...
        while iterations < self._max_tool_iterations:
            ...
            if completion.has_tool_calls:
                ...
                # Filter tool results to remove error noise
                filtered_tool_results = self._filter_tool_results(tool_results)

                # Check for overall success
                successful_tools = sum(1 for t in tool_results if t.get("result", {}).get("success"))
                total_tools = len(tool_results)
                quality = 0.8 if total_tools == 0 else (0.9 * successful_tools / total_tools + 0.1)
                ...
                return self._create_result(
                    ...
                    result={
                        ...
                        "tool_executions": filtered_tool_results,
                    },
                    confidence=quality,
    ...
    def _filter_tool_results(self, tool_results: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Filter tool results to remove error noise and permission messages."""
        filtered = []
        for result in tool_results:
            result_data = result.get("result", {}) or {}

            # Check if this is an error result with permission issues
            error = result_data.get("error") or ""
            if error and ("permission" in error.lower() or "not have permission" in error.lower()):
                ...
                filtered.append({
                    ...
                    "result": {
                        "success": False,
                        "error": "Tool not available for this operation",
                    }
                })
            else:
